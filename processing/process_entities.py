...
pd.options.mode.chained_assignment = None  # Disables chained assignment warnings - not applicable here

code_list_df = pd.read_csv('https://storage.googleapis.com/ajah_contracts/nonprofitknowledge/entities/program_codes_list.csv', index_col=0)
shortened = False
print("Setting up...")

def fetch_df(file, shortened=shortened):
...
general_info_df = fetch_df('https://storage.googleapis.com/ajah_contracts/nonprofitknowledge/entities/financial_section_a_b_and_c_2019.csv')
id_df = fetch_df('https://storage.googleapis.com/ajah_contracts/nonprofitknowledge/entities/ident_2019.csv')
financials_df = fetch_df('https://storage.googleapis.com/ajah_contracts/nonprofitknowledge/entities/financial_d_and_schedule_6_2019.csv')
compensation_df = fetch_df('https://storage.googleapis.com/ajah_contracts/nonprofitknowledge/entities/schedule_3_compensation_2019.csv')
web_df = fetch_df('https://storage.googleapis.com/ajah_contracts/nonprofitknowledge/entities/weburl_2019.csv')
...
def process_scraper_output(df):
    ...
    cleaned['website_text'] = cleaned['website_text'].apply(lambda x: x.lower()).str.replace('menu','').str.replace('home','')
...
def main():
  ...
  final_df.drop('website_text',axis=1).to_csv('./../data/processed/sks_entities_public_{}.csv'.format(timestamp),index=False,header=True)
